...
def prepare_initial_leaf_embeddings(msa_paths, Large_D = 1000, model = None, batch_converter = None, device = "cuda", use_transf = True, alphabet = None):
    ...
    for msa_path in tqdm(msa_paths):
        ...
        if use_transf:
            ...
            embeddings = torch.zeros(len(msa), len(msa[0][1]), len(alphabet_dict))
...
def make_datasets(msa_path, tree_path):
    

    all_branch_lengths, all_root_distances, seqs_order = prepare_branch_lengths_and_root_distances(tree_path, msa_path)
