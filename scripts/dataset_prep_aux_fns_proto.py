...
def prepare_initial_int_node_embeddings(MSA_file_path, seqs_order, Large_D = 768, leaf_embeddings = None, delete_init_tree = True, fam = None):
...
def prepare_initial_leaf_embeddings(msa_paths, Large_D = 768, model = None, batch_converter = None, device = "cuda", use_transf = True, alphabet = None):
    
    all_embeddings = []

    for msa_path in tqdm(msa_paths):
        ...
        if use_transf:
            ...
            embeddings = torch.zeros(len(msa), len(msa[0][1]), len(alphabet_dict) if len(alphabet_dict) % 2 == 0 else len(alphabet_dict) + 1).to(device)
...
def prepare_initial_leaf_embeddings_simple(msa_paths, Large_D = 768, model = None, batch_converter = None, device = "cuda", alphabet = None):
    ...
    leaf_embedder = torch.nn.Embedding.from_pretrained(msa_transf_token_embedding_weights, freeze=True).to(device)
    ...
    for msa_path in tqdm(msa_paths):
        ...
        with torch.no_grad():
            embeddings = leaf_embedder(tokens)[0][:,1:]
            embeddings = embeddings.mean(dim=1).cpu()
            R, H = embeddings.shape
            padding_tensor = torch.zeros((R, Large_D - H))
            embeddings = torch.concat((embeddings, padding_tensor), dim=1)
...
def make_datasets(MSA_path, true_tree_path, leaf_embeddings = None, Large_D = 768):
...
def make_datasets_no_internal(MSA_path, true_tree_path, leaf_embeddings = None, Large_D = 768):
    
    all_branch_lengths, mapped_parents, seqs_order = prepare_branch_lengths_and_relationships(true_tree_path, MSA_path)

    return all_branch_lengths, mapped_parents
